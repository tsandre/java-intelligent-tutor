...
public class jGuruScrape {
		public static LinkedHashSet <ScrapeData> ScrapeQuery(String queryText){
			String websiteName = "jguru.com";
			String scrapeSource = "jGuru";
			LinkedHashSet <ScrapeData> jGuruScrapeObj = new LinkedHashSet <ScrapeData> ();
			...
			try {
				...
				GoogleResponse gResponse = customSearchInstance.getSearchResults(queryText+" site:jguru.com");
				//Call Restoration
				...
				for (Item item : gResponse.getItems()) {
					...
					scrapeDataObj.setInfoQuery(queryText);
			  		...
			  		jGuruScrapeObj.add(scrapeDataObj);
					}
				}
				}
			} catch (Exception e) {			
			...
			System.out.println("Result count of jGuru :"+jGuruScrapeObj.size());
			return jGuruScrapeObj;
		}
} 